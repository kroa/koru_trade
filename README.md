...
| **`config/frequent.example.yaml`** | **신호 빈도 확대** — 진입 필터 4개 완화 | **77건 · +23.27% · PF 1.87** |
...
`frequent` 가 현재 검증된 것 중 가장 낫다. **부트스트랩 95% 신뢰구간이
+0.14% ~ +3.12% 로 0을 넘지 않는 유일한 설정**이다(다른 프리셋은 전부 0을 포함한다).
워크포워드 4구간 중 3구간 수익, 파산위험 0.3%.

**신호 빈도의 상한**은 주 2.9회다(평균 보유 1.7일 / 주 5거래일, 동시 보유 1개).
필터를 전부 꺼도 실측은 주 0.78회에 그치고, 그 지점에서 PF 가 1.87 -> 1.16 으로,
최대낙폭이 -5.6% -> -12.0% 로 나빠진다. 빈도를 사면 수익을 판다.
